package ru.nkz.ivcgzo;

public class configuration {
	public static final String serverClassName = "ru.nkz.ivcgzo.serverRegPatient.ServerRegPatient";
	public static final String clientClassName = "ru.nkz.ivcgzo.clientRegPatient.MainForm";
	public static final int thrPort = 55005;
	public static final int appId = 5;
	public static final String appName = "Информация о пациентах больницы";
	public static final String appVersion = "0.0.0.0";
}
