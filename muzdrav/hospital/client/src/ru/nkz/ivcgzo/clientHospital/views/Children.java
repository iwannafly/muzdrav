...
//  просто сделал старый конструктор методом setChildrenPanel()
 ...
 */
public final class Children extends JPanel implements IPatientObserver {
    private static final long serialVersionUID = 3513837719265529746L;
    private static final String TOOLTIP_TEXT =
            "<html><b>Новорожденные</b> - позволяет лечащему врачу сохранять информацию "
            + "о новорожденных</html>";
    private static final URL ICON = MainFrame.class.getResource(
            "/ru/nkz/ivcgzo/clientHospital/resources/children.png");
    private static final String TITLE = "Новорожденные";
    ...
    private JTextField tfDocName;
     ...
     public Children(final ChildrenController inController,
            final IHospitalModel inModel) {
        this.controller = inController;
        this.model = inModel;
        // регистрация слушателя модеи. позволяет реагировать на смену пациента
        model.registerPatientObserver((IPatientObserver) this);

        setChildrenPanel(model.getPatient());
    }
...
//	}
    ...
    // бывший конструктор
    public void setChildrenPanel(final TPatient patientInfo) {
        this.userAuth = ClientHospital.authInfo;
        // Вообще выбранного пациента в любом месте программы теперь можно получить
        // через интерфейсную ссылку на модель (model.getPatient())
        this.patient = patientInfo;
        //Инициализация интерфейса:
        this.setInterface();
        this.updatePanel(); //Обновление панели
    ...
    public String getPanelToolTipText() {
        return TOOLTIP_TEXT;
    }
	...
	/**
	 * Вызывается при смене пациента в модели (выборе пациента)
	 */
    @Override
