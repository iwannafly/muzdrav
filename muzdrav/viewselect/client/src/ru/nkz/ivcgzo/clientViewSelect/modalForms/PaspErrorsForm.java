...
public class PaspErrorsForm extends ModalForm {
	private static final long serialVersionUID = 1370164410667616330L;
	private CustomTable<PaspErrorInfo, PaspErrorInfo._Fields> tblErr;
	...
	public PaspErrorsForm() {
		super(true);
		
		setTitle("Ошибки паспортной части");
		...
		btnSrc.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					lblDesc.setText("");
					tblErr.setData(MainForm.tcl.getPaspErrors(MainForm.authInfo.cpodr, tbSrcFrom.getTime(), tbSrcTo.getTime()));
					...
					JOptionPane.showMessageDialog(PaspErrorsForm.this, "Не удалось получить результаты поиска.", "Ошибка", JOptionPane.ERROR_MESSAGE);
				} catch (TException e1) {
					e1.printStackTrace();
		...
		tblErr = new CustomTable<>(false, true, PaspErrorInfo.class, 1, "Номер", 2, "Фамилия", 3, "Имя", 4, "Отчество", 5, "Дата рождения", 6, "Код ошибки");
	...
	public void acceptResults() {
		if (tblErr.getSelectedItem() != null)
			results = tblErr.getSelectedItem().npasp;
		else
			return;
