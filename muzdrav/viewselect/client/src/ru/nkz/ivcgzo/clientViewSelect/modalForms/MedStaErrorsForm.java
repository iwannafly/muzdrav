...
public class MedStaErrorsForm extends ModalForm {
	private static final long serialVersionUID = 3871310888472373862L;
	private CustomTable<MedStaErrorInfo, MedStaErrorInfo._Fields> tblErr;
	...
	private boolean priem;
	
	public MedStaErrorsForm() {
		super(true);
		...
		btnSrc.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					lblDesc.setText("");
					if (priem)
						tblErr.setData(MainForm.tcl.getMedPriemErrors(MainForm.authInfo.cpodr, tbSrcFrom.getTime(), tbSrcTo.getTime()));
					else
						tblErr.setData(MainForm.tcl.getMedStaErrors(MainForm.authInfo.cpodr, tbSrcFrom.getTime(), tbSrcTo.getTime()));
					...
					JOptionPane.showMessageDialog(MedStaErrorsForm.this, "Не удалось получить результаты поиска.", "Ошибка", JOptionPane.ERROR_MESSAGE);
				} catch (TException e1) {
					e1.printStackTrace();
		...
		tblErr = new CustomTable<>(false, true, MedStaErrorInfo.class, 2, "Дата госп.", 3, "Номер пациента", 4, "ФИО пациента", 5, "ДР пациента", 6, "Код ошибки");
		tblErr.setDateField(0);
		tblErr.setDateField(3);
	...
	public void acceptResults() {
		if (tblErr.getSelectedItem() != null) {
			results = (priem) ? tblErr.getSelectedItem().id_gosp : tblErr.getSelectedItem().id_otd;
		} else {
			return;
		}
	...
	public void setPriem(boolean val) {
		if (priem != val) {
			tblErr.setData(new ArrayList<MedStaErrorInfo>());
			lblDesc.setText(null);
		}
		priem = val;
		
		if (priem)
			setTitle("Ошибки медицинской части приемного отделения");
		else
			setTitle("Ошибки медицинской части стационара");
	}
}
