package ru.nkz.ivcgzo;

public class configuration {
	public static final String serverClassName = "ru.nkz.ivcgzo.serverMss.serverMss";
	public static final String clientClassName = "ru.nkz.ivcgzo.clientMss.MainForm";
	public static final int thrPort = 55006;
	public static final int appId = 6;
	public static final String appName = "Медицинское свидетельство о смерти";
}
